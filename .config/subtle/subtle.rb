...
my_font = "-*-gohufont-medium-r-*-*-11-*-*-*-c-*-iso8859-*"
set :font, "-*-gohufont-medium-r-*-*-11-*-*-*-c-*-iso8859-*"
#set :font, "xft:sans-8"
...
grab "W-S-f", :WindowStick

# Toggle zaphod mode of window (will span across all screens)
grab "W-equal", :WindowZaphod

# Raise window
grab "W-C-h", :WindowRaise

# Lower window
grab "W-C-t", :WindowLower

# Select next windows
grab "W-d",  :WindowLeft
grab "W-h",  :WindowDown
grab "W-t",    :WindowUp
grab "W-n", :WindowRight

# Kill current window
grab "W-C-c", :WindowKill

# Cycle between given gravities
grab "W-S-g", [ :top_left,     :top_left66,     :top_left33     ]
grab "W-S-t", [ :top,          :top66,          :top33          ]
grab "W-S-c", [ :top_right,    :top_right66,    :top_right33    ]
grab "W-S-d", [ :left,         :left66,         :left33         ]
grab "W-S-s", [ :center,       :center66,       :center33       ]
grab "W-S-n", [ :right,        :right66,        :right33        ]
grab "W-S-m", [ :bottom_left,  :bottom_left66,  :bottom_left33  ]
grab "W-S-h", [ :bottom,       :bottom66,       :bottom33       ]
grab "W-S-w", [ :bottom_right, :bottom_right66, :bottom_right33 ]
...
# Exec programs
grab "W-r", 'dmenu_run -fn " + my_font + " -nb "#292926" -nf "#eeeeee" -sb "#0066ff" -sf "#eeeeee"'
grab "W-Return", "urxvt"
...
tag "window",  "pcmanfm|evince|viewnior"
tag "chat",    "pidgin"
tag "music",   "spotify"
tag "download","deluge|rtorrrent|xftp"
tag "mail",    "thunderbird|claws-mail"
...
view "term" do
  match "term|mpd|tmux"
  dynamic false
  icon "#{ENV["HOME"]}/.config/subtle/icons/term.xbm"
  icon_only true
end
...
view "www" do
  match "browser"
  dynamic false
  icon "#{ENV["HOME"]}/.config/subtle/icons/earth.xbm"
  icon_only true
end
...
view "etc" do
  match "default|gimp_.*"
  dynamic false
  icon "#{ENV["HOME"]}/.config/subtle/icons/ghost.xbm"
  icon_only true
end
...
view "window" do
  match "window"
  dynamic false
  icon "#{ENV["HOME"]}/.config/subtle/icons/win.xbm"
  icon_only true
end
...
view "chat" do
  match "chat"
  dynamic false
  icon "#{ENV["HOME"]}/.config/subtle/icons/chat.xbm"
  icon_only true
end
...
view "dev" do
  match "editor|vim"
  dynamic false
  icon "#{ENV["HOME"]}/.config/subtle/icons/bug_02.xbm"
  icon_only true
end
...
view "music" do
  match "music"
  dynamic false
  icon "#{ENV["HOME"]}/.config/subtle/icons/phones.xbm"
  icon_only true
end
...
view "download" do
  match "download"
  dynamic false
  icon "#{ENV["HOME"]}/.config/subtle/icons/diskette.xbm"
  icon_only true
end
