...
map.dir('h', alias='<down>')
map.dir('t', alias='<up>')
map.dir('d', alias='<left>')
map.dir('n', alias='<right>')
map.dir('gg', alias='<home>')
map.dir('G', alias='<end>')
map.dir('<C-D', alias='<pagedown>')
map.dir('<C-U>', alias='<pageup>')
...
map('<C-D>', 'H', fm.move(down=0.5, pages=True))
map('<C-U>', 'T', fm.move(up=0.5, pages=True))
...
map('y ', fm.copy(mode='remove'))
map('jj', 'j<dir>', fm.cut())
map('ja', fm.cut(mode='add'))
map('jr', fm.cut(mode='remove'))
map('j ', fm.cut(mode='remove'))
map('pp', fm.paste())
...
# -------------------------------------------------------- movement
map('<left>', 'd', wdg.move(left=4))
map('<right>', 'n', wdg.move(right=4))
map('<C-D>', 'h', wdg.move(down=0.5, pages=True))
map('<C-U>', 't', wdg.move(up=0.5, pages=True))
...
map.merge(global_keys)
map.merge(vim_aliases)
map('H', wdg.task_move(0))
map('T', wdg.task_move(-1))
map('jjy', wdg.task_remove())
